...
{
    public sealed class Sphere3D2:Primitive3D
    {

        internal Point3D GetPosition(double t, double y)
        {
            double s = Math.Acos(y);
            double x = Math.Sin(s) * Math.Sin(t) + Math.Cos(20 * s) / 20;
            double z = Math.Sin(s) * Math.Cos(t) + Math.Cos(20 * t) / 20;
